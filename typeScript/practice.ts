...
function palindromCheck(numb: number | string): boolean {
  let org = numb.toString().toLowerCase();
  let con = org.split("").reverse().join("");

  return org === con;
}

console.log(
  palindromCheck(number) ? `${number} is palindrom` : `${number} is not`
);
